            ...
            <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody
                            v-show="!TagStore.state.loading"
                            class="divide-y divide-gray-200 bg-white"
                        >
                            <tr v-if="TagStore.state.companies.length == 0">
                                <td
                                    colspan="5"
                                    class="text-center text-xl text-red-800"
                                >Nenhuma TAG encontrada</td>
                            </tr>
                            <tr v-else v-for="tag in TagStore.state.companies" :key="tag.id">
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    <div class="text-gray-900">{{ tag.description }}</div>
                                </td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    <span
                                        v-if="tag.active"
                    ...
                    <div v-show="TagStore.state.loading" class="text-center">
                        <p class="mb-2 mt-2 text-xl text-primary">Carregando</p>
                        <p class="mb-2">
                            <pulse-loader
                                :loading="TagStore.state.loading"
                                color="#16A34A"
                                size="16px"
                            ></pulse-loader>
                        </p>
...
function del(tag) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            TagStore.dispatch('delete', tag)
        }
    })
}

function edit(id) {
    TagStore.dispatch('edit', id)
}
